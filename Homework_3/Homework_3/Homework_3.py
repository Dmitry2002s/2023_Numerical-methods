...
f = 1 - sympy.exp(r) - r**2 + r**3 
def function(f,X):
    return f.evalf(subs = {r : X})  #1 - math.exp(-x) + x**2 #Вычисление значения функции 
...
p = a 
for i in range(0,m):
    Table.append( cell( p + (b - a )*i / (m-1), function(f, p  + (b - a  ) * i / (m-1))) ) #Заполняем функцию равноотстоящими значениями 
for i in Table: 
...
f_diff = f.diff(r)  #Посчитали производную функции 

#print(f_diff)
f_diff2 = f_diff.diff(r)
...
while (X2 <= b):##Находим, где меняется знак и считаем число корней
    ...
    Y1 = Y2 
monotony = [] 
E = 0.00001#Эпсилон допуск для поиска участков немонотонности 
if (len(List) == 0):
    ...
    for i in List: 
        ...
        while (abs( B - A ) > 2 * E):
            C = B-(B-A)*f_diff.evalf(subs ={r:B})/(f_diff.evalf(subs ={r:B}) - f_diff.evalf(subs ={r:A}))
            A = B 
            B = C 
        ...
        if(len(monotony)==0):
            ...
            monotony.append(interval(monotony[len(monotony) - 1 ].X2, C))
...
while (choise == 1) :
    ...
    for m in monotony: 
        ...
        while (n>=len(INTER)):
                print("Требуемая степень превышает или равно числу значений функции в таблице для данного промежутка монотонности\nВыбрана максимально доступная степень многочлена = ", len(INTER) - 1 )
                n = len(INTER) - 1 
        #for i in INTER: 
        #    i.print() 
        ...
        def nearest(x, monotony = monotony,List = INTER):
            result = []
            MIN = 2147483647
            ...
            for k in monotony: 
                for i in List :
                    if(i.number>k.X1 and i.number<k.X2 ):
        ...
        B = nearest(x) #Хранит ближайший к точке интерполяции узел из каждого промежутка монотонности 
        ...
        for Z in B: 
            ...
            print("Разница интерполяционного многочлена и функции  = ", abs(function(f,result_L1)-x), "\n")
    ...
    Z = nearest(x, Table, monotony) #Хранит ближайший к точке интерполяции узел 
    '''
    for i in monotony:
    ...
    A = a
    B = b
    N = 500 #Количество отрезков, котоыре проверяем на наличие корня нечётной кратности 
    ...
    for i in AAAAA:
        ...
        print("Абсолютная величина невязки -", abs(function(f,C) - x) , "\n") #Метод секущих
